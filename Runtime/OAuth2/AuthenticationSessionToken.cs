...
{
    public class AuthenticationSessionToken : IDisposable
    {
        ...
        public AuthenticationSessionToken(AuthorizationCodeFlow client, IBrowser browser)
        {
            _client = client;
            _browser = browser;
        }
        ...
        public async Task<RequestTokenResponse> AuthenticateAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                if (browserResult.status == BrowserStatus.Success)
                {
                    ...
                    resp.SetResponse(browserResult.redirectUrl, _client.configuration.redirectUri);
                    return resp;
                }
